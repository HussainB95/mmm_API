...
def create_slot(data: CreateSlotRequest, db: Session = Depends(get_db)):
...
def get_available_slots(practitioner_id: str, db: Session = Depends(get_db)):
...
def book_slot(data: BookSlotRequest, db: Session = Depends(get_db)):
