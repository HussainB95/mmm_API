...
router = APIRouter(prefix="/practitioners", tags=["Practitioner Profile"])
...
#Update
@router.put("/{practitioner_id}")
def update_profile(practitioner_id: str, data: ProfileData, db: Session = Depends(get_db)):

    return practitioner_profile_service.update_section(db, practitioner_id, "profile_data", data.model_dump())
